...
jsval_t js_mkarr(struct js *, size_t);                         // Create array
void js_set(struct js *, jsval_t, const char *, jsval_t);      // Set obj attr
size_t js_arrsize(struct js *, jsval_t);
jsval_t js_arrget(struct js *, jsval_t, size_t);
jsval_t js_arrset(struct js *, jsval_t, size_t, jsval_t);

// Extract C values from JS values
enum { JS_UNDEF, JS_NULL, JS_TRUE, JS_FALSE, JS_STR, JS_NUM, JS_ARR, JS_ERR, JS_PRIV };
