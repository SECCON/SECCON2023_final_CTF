...
enum {
  ...
  T_OBJ, T_PROP, T_STR, T_ARR, T_UNDEF, T_NULL, T_NUM, T_BOOL, T_FUNC, T_CODEREF,
  T_CFUNC, T_ERR
};

static const char *typestr(uint8_t t) {
  const char *names[] = { "object", "prop", "string", "array", "undefined", "null",
...
size_t js_arrsize(struct js *js, jsval_t v) { return *(size_t*)&js->mem[(jsoff_t) vdata(v)]; }
jsval_t js_arrget(struct js *js, jsval_t v, size_t i) { return ((jsval_t*)(&js->mem[(jsoff_t) vdata(v)] + sizeof(size_t)))[i]; }
jsval_t js_arrset(struct js *js, jsval_t v, size_t i, jsval_t new) { return ((jsval_t*)(&js->mem[(jsoff_t) vdata(v)] + sizeof(size_t)))[i] = new; }
...
static size_t tostr(struct js *js, jsval_t value, char *buf, size_t len) {
  switch (vtype(value)) {  // clang-format off
    ...
    case T_ARR:   return (size_t) snprintf(buf, len, "ARRAY(size:%lu)", js_arrsize(js, value));
...
jsval_t js_mkarr(struct js *js, size_t len) {
  // Create buffer
  if (len > 0x7fffffffUL) return js_mkerr(js, "size too big");
  jsval_t value = mkentity(js, 0 | T_ARR, NULL, sizeof(size_t) + len * sizeof(jsval_t));
  if (is_err(value)) return value;

  // Set length
  jsoff_t off = (jsoff_t) vdata(value);
  *(size_t*) &js->mem[off] = len;

  // Set default elements
  jsval_t *arr = (jsval_t*) (&js->mem[off] + sizeof(size_t));
...
static jsval_t do_dot_op(struct js *js, jsval_t l, jsval_t r) {
  ...
  // Handle arrayvalue.length
  if (vtype(l) == T_ARR && streq(ptr, codereflen(r), "length", 6)) {
    return tov(*(size_t*)&js->mem[(jsoff_t) vdata(l)]);
  }
